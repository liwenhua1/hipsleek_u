...
let rec check_prover_existence prover_cmd_str =
  ...
  | prover::rest -> 
    ...
    let prover = 
      ...
      else if String.compare prover "mona" = 0 then try FileUtil.which "mona" with Not_found | Unix.Unix_error _ -> ""
      else prover
    in
...
let set_tp user_flag tp_str =
  ...
  (**********************************************)
  let redcsl_str = if !Globals.web_compile_flag then try FileUtil.which "redcsl" with Not_found | Unix.Unix_error _ -> "" else "redcsl" in
